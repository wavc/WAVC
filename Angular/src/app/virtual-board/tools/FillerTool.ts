...
export class FillerTool extends Tool {
    ...
    constructor(private signaRConnection:signalR.HubConnection, fillColor: Color, project: Project, hitOptions: any) {
        super();
    ...
    onMouseDown = (event) => {
        var hitResult = this.project.hitTest(event.point, this.hitOptions);
        if(this.tryFill(hitResult, this.fillColor)){
            this.signaRConnection.send("SendOnMouseDownEvent", this.myId, ["fill", event.point, this.fillColor]);
        };
